...
El __port forwarding__, también conocido como reenvío de puertos, nos permite redirigir el tráfico de red desde un puerto específico en el host a un puerto específico en el contenedor. Esto nos permitirá acceder a los servicios que se ejecutan dentro del contenedor desde el exterior.
...
- docker run -p 80:8080 mi_imagen
    - docker run -p 80:80 --name myWebserver webserver
    - lsof -i:80
    - docker logs [ID] -> puedes ver lo que ha pasado..., _-f_= ayuda a ver en tiempo real [!]
...
- docker run -v /home/usuario/datos:/datos mi_imagen
    - docker run -dir -p 80:80 -v /home/tiaago/desktop/docker/:/var/www/html --name myWebserver webserver
    - docker exec -it myWebserver bash -> entras en la bash del contenedor
